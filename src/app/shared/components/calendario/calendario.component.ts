...
@Component({
  ...
  template: `
    <div class="cal">
      <div class="cal__head">
        <button class="cal__nav" (click)="prevMes()" [disabled]="!puedeRetroceder()" aria-label="Mes anterior">‹</button>
        <h3 class="cal__mes">{{ nombreMes() }} {{ anio() }}</h3>
        <button class="cal__nav" (click)="nextMes()" aria-label="Mes siguiente">›</button>
      </div>
      ...
      <ul class="cal__leyenda">
        <li><span class="lg estado-disponible"></span>Disponible</li>
        <li><span class="lg estado-con-tour"></span>Con tour</li>
        <li><span class="lg estado-ocupado"></span>Ocupado</li>
        <li><span class="lg estado-no-disponible"></span>No disponible</li>
      </ul>

      <!-- Franjas del día elegido -->
      <div class="cal__franjas" *ngIf="diaElegido() as dia">
        <h4>{{ formatoLargo(dia.iso) }}</h4>
        <p class="cal__pick">Elige una franja para reservar:</p>
          ...
          <div class="slot" *ngFor="let f of dia.franjas">
            ...
            <a
              *ngIf="whatsapp; else sinWa"
              ...
              rel="noopener"
            >Reservar</a>
            <ng-template #sinWa><span class="slot__nowa">Configura tu WhatsApp</span></ng-template>
          </div>
        </div>
      </div>
    </div>
  ...
  styles: [`
    ...
    .cal__nav { width: 38px; height: 38px; border-radius: 50%; border: 1px solid var(--linea); background: #fff; font-size: 1.3rem; line-height: 1; cursor: pointer; color: var(--tinta); }
    ...
    .estado-disponible:hover { background: rgba(244,183,64,.32); transform: translateY(-1px); }
...
export class CalendarioComponent implements OnInit {
  ...
  waLink(iso: string, f: Franja): string {
    ...
    const msg = `Hola, quiero reservar la ruta "${this.tituloRuta(f.rutaId)}" el ${fecha} a las ${f.hora}. ¿Confirmamos?`;
    return `https://wa.me/${this.whatsapp}?text=${encodeURIComponent(msg)}`;
  }

  etiqueta(c: Celda): string {
    const map: Record<EstadoDia, string> = {
      'disponible': 'disponible', 'con-tour': 'con tour', 'ocupado': 'ocupado',
      'no-disponible': 'no disponible', 'sin-tours': 'sin tours', 'pasado': 'pasado',
    };
